...
class ContainerMonitoring:
    ...
    def Setup(self):
        logger.info("Creating necessary folder structure...")
        try:
            ...
            logger.error("File is not exists or already removed, error reason", exp)
            return False
    ...
    def FetchContainerStatistics(self):
        logger.info("Inside FetchContainerStatistics...")
        containerDict = {}
            ...
            while True:
                ...
                while(index < len(allContainerList)):
                    try:
                        ...
                        logger.info("Writing log file for container:", self.trimCharacters(containerName))
                        stats = allContainerList[index].stats(stream=False)
                        if not self.writeContainerStatistics(containerName, stats):
                            logger.info("Error while writing log statistics for container " + containerName)
                    except Exception as newExp:
                        logger.info("Unable to fetch statistics...reason:", newExp)
                    index = index + 1;
                    
                #make sure to write in some interval
                time.sleep(intervalInSecond)
        except Exception as exp:
            logger.error("Error while finding list of the docker container...reason:", exp)

    def writeContainerStatistics(self, containerName, statsObj):
        logger.info("Inside writeContainerStatistics...")
        # logger.info(statsObj)
        try:
            ...
            logger.error("Failed to write statistics.", newExp)
            return False
...
if __name__ == "__main__":
    ...
    if cMonitoring.Setup():
        logger.info('Capturing container reports...')
